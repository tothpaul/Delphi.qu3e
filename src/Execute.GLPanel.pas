...
function ShaderTexture(Width, Height: Integer; const PixelShader: AnsiString): Cardinal;
...
var
  ...
  Buffers: Cardinal;
  Shader: Cardinal;
  fsize : array[0..1] of Single;
begin
  Shader := glxCompileProgram(VERTEX_SHADER, PixelShader);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, Width, Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nil);
  ...
  var size := glGetUniformLocation(Shader, 'size');
  if size >= 0 then
  begin
    var glLen: GLsizei;
    var glSize: Integer;
    var glType : GLenum;
    glGetActiveUniform(Shader, size, 0, @glLen, @glSize, @glType, nil);
    if glType = GL_FLOAT_VEC2 then
    begin
      fsize[0] := width;
      fsize[1] := height;
      gluniform2fv(size, 1, @fsize);
    end;
  end;
...
var
  ...
  wglChoosePixelFormatARB : function(DC: HDC; const Attr2iv; const Attri2fv; MaxFormat: Integer; var Formats, FormatCount: Integer): Boolean; stdcall;
