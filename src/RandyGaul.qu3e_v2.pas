...
// math/q3Vec3.h
  ...
  q3Vec3 = record
    ...
    procedure Identity;
    function Dot(const v: q3Vec3): r32;
    function Cross(const v: q3Vec3): q3Vec3;
    function Length: r32;
    ...
    class operator Multiply(const a, b: q3Vec3): q3Vec3;
...
type
  ...
  private
    m_memory: TArray<Byte>;
    m_entries: TArray<q3StackEntry>;
    m_index: u32;
    m_allocation: i32;
    m_entryCount: i32;
    m_entryCapacity: i32;
  ...
  private
    m_memory: pq3Header;
    m_freeBlocks: TArray<q3FreeBlock>;
...
begin
  Result.Create(System.Math.Min(x, v.x), System.Math.Min(y, v.y), System.Math.Min(z, v.Z));
end;

function q3Vec3.Max(const v: q3Vec3): q3Vec3;
begin
  Result.Create(System.Math.Max(x, v.x), System.Math.Max(y, v.y), System.Math.Max(z, v.Z));
end;
...
begin
  m_entryCapacity := 64;
  SetLength(m_entries, m_entryCapacity);
end;
...
begin
  ...
  begin
    m_stackSize := size;
    SetLength(m_memory, m_stackSize);
  end;
end;
...
begin
  ...
  SetLength(heap.m_freeBlocks, heap.m_freeBlockCapacity);

  heap.m_freeBlocks[0].header := heap.m_memory;
  heap.m_freeBlocks[0].size := q3k_heapSize;
end;

class operator q3Heap.Finalize(var heap: q3Heap);
begin
  q3Free(heap.m_memory);
  heap.m_freeBlocks := nil;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SetLength(m_freeBlocks, m_freeBlockCapacity);
      end;
...
function q3Combine(const a, b: q3AABB): q3AABB;
begin
  Result.min := a.min.Min( b.min);
  Result.max := a.max.Max( b.max);
end;
...
begin
  ...
  SetLength(tree.m_nodes, tree.m_capacity);
...
begin
  tx.position.Identity;
  tx.rotation.Identity;
end;
...
begin
  ...
  begin
    var a: q3Vec3 := q3Mul( world, vertices[ kBoxIndices[ i ] ] );
		...
		var n: q3Vec3 := ( b - a).Cross( c - a ).Normalize;
